...
#define NUMBER_FORMAT MAKE_NUMBER_FORMAT_1(MAX_DIGITS)
      ...
      _snprintf_s(raw_result, sizeof(1024), header, start_html_offset,
...
#undef NUMBER_FORMAT
    ...
    const CLIPFORMAT moz_url_format = (CLIPFORMAT)::RegisterClipboardFormat(L"text/x-moz-url");
    const CLIPFORMAT html_format = (CLIPFORMAT)::RegisterClipboardFormat(L"HTML Format");
    const CLIPFORMAT file_desc_format = (CLIPFORMAT)::RegisterClipboardFormat(CFSTR_FILEDESCRIPTOR);
  ...
  CefBrowserHost::DragOperationsMask DropTargetWin::StartDragging(
      ...
      int y)
  {
    LOG(INFO) << "StartDragging from osr file started current thread is " << GetCurrentThreadId();
    ...
    if (DragDataToDataObject(drag_data, &dataObject))
    {
      ...
      DoDragDrop(dataObject, dropSource, effect, &resEffect);
      LOG(INFO) << "StartDragging from osr file finished";
